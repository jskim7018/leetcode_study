...
class Solution:
    def minEdgeReversals(self, n: int, edges: List[List[int]]) -> List[int]:
        # start from one that is leaf (if all is bi-directional)
        # go to next ones. (if outward no cost, if inward then cost 1)
        # also add next's total cost to curr.
        # also make sure to not check prev (one where it came from.
